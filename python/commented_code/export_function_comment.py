...
# (BMR) Note that below prefixes are used as [prefix].[what comes after]
G.bind('LDP', LDP)
G.bind('RDA', RDA)
# (BMR) Code I don't understand -- this assigns an empty (?) array to the URIS var??
URIS = [] # list for record uris

# (BMR) Okay here's a piece of code I don't understand
    # I'm wondering if this gets the triples for each of the trellis resources which are the objects of triples at the 'endpoint' location above
...
# (BMR) Does this happen before serialized and exported as ttl and xml? Need to run code to find out.
    # How does this work?
    # I don't think this happens before saving of serializations above
"""concatenates individual records into singular graph"""
g = Graph()
for uri in URIS:
    g.parse(uri)

# (BMR) Here is the code I'm not going to be using at this point --
    # right now I only need to serialize and save RDF/XML locally for transformation
